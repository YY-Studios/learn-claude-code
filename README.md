...
### 02. 컨텍스트 관리 전략
- [컨텍스트 윈도우 이해하기](./02-context/context-window.md) — 한 세션 = 한 피처 원칙
- [CLAUDE.md 계층 구조](./02-context/claude-md-hierarchy.md) — 폴더별 분리, Lazy Loading
- [Second Brain 구축](./02-context/second-brain.md) — docs/decisions.md로 의사결정 축적
- [/memory 자동 메모리](./02-context/memory.md) — Claude가 학습한 내용 자동 저장·로드
- [Mermaid 아키텍처](./02-context/mermaid.md) — 다이어그램으로 프로젝트 구조 정리

### 03. 실전 워크플로우
- [Plan Mode 활용](./03-workflow/plan-mode.md) — Shift+Tab으로 계획 먼저, 리뷰 후 실행
- [TDD 기반 코딩](./03-workflow/tdd.md) — 작은 변경 → 테스트 → 린트 → 커밋 → 반복
- [TODO.md 활용](./03-workflow/todo-md.md) — 작업 연속성 유지 & 병렬 처리
- [에러 로그 전달법](./03-workflow/error-logs.md) — 해석하지 말고 통째로 전달
...
- [Sub-agent & 병렬 처리](./04-agents/subagents.md) — Agent Teams로 작업 분산
...
### 06. 실전 패턴 (Supabase)
- [Supabase 스키마 설계 맡기기](./06-supabase/schema.md)
- [Edge Function 작성](./06-supabase/edge-functions.md)
- [프론트엔드 훅 생성](./06-supabase/frontend-hooks.md)
